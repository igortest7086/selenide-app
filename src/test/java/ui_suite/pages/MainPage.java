...
public class MainPage extends AbstractPage{
    ...
    public MainPage clickLogInButton() {
        $x(String.format(SIGN_IN_OR_UP_BUTTON_TEMPLATE, SignInUpType.SIGN_IN)).shouldBe(enabled, Duration.ofSeconds(10)).click();
        return this;
    }

    @Step("Check option ''{0}'' in Header menu is present")
    public MainPage checkOptionInHeaderMenuIsPresent(HeaderMenuType option) {
        $x(String.format(HEADER_MENU_TEMPLATE, option)).shouldBe(appear, Duration.ofSeconds(10));
        return this;
    }

    @Step("Hover on option ''{0}'' in Header menu")
    public MainPage hoverOnOptionInHeaderMenu(HeaderMenuType option) {
        $x(String.format(HEADER_MENU_TEMPLATE, option)).shouldBe(enabled, Duration.ofSeconds(10)).hover();
        return this;
    }

    @Step("Click on Pricing in Header menu")
    public MainPage clickOnPricingInHeaderMenu() {
        $x(String.format(HEADER_MENU_TEMPLATE, HeaderMenuType.PRICING)).shouldBe(enabled, Duration.ofSeconds(10)).click();
        return this;
    }

    public String getLoggedInUserName()
    {
        $(USER_LOGGED_AVATAR).shouldBe(enabled, Duration.ofSeconds(10)).click();
        return $(LOGGED_USER_NAME).shouldBe(enabled, Duration.ofSeconds(10)).getText();
